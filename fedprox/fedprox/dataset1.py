...
import os
from fedprox.dataset_preparation1 import _partition_data,build_transform, create_domain_shifted_loaders,buid_domain_transform, DataSplitManager,create_pathmnist_scenario2_loaders
import medmnist
...
def load_datasets(  # pylint: disable=too-many-arguments
  ...
  print(f"Dataset partitioning config: {domain_shift}")
  ...
  if dataset=="pathmnist":
      ...
      data_flag = 'pathmnist'
      info      = INFO[data_flag]
      DATA_DIR  = os.path.join(os.getcwd(), data_flag)
      os.makedirs(DATA_DIR, exist_ok=True)

      # 1) Download into DATA_DIR
      _ = PathMNIST(split='train',  root=DATA_DIR, download=True)
      _ = PathMNIST(split='test',   root=DATA_DIR, download=True)
      
      # 2) Build npz_path from INFO['filename']
      npz_path = os.path.join(DATA_DIR, "pathmnist.npz")
      print(f'ddd {npz_path}')
      del _
      import gc; gc.collect()
      
      # Locate the .npz on disk
      trainloaders, valloaders,testloaders = create_pathmnist_scenario2_loaders(
      npz_path ,
   ...
   if domain_shift==True:
      ...
      # Print shapes to inspect the data
